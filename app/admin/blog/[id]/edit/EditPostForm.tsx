...
import { ArrowLeft, Upload, Loader2, Save, Info, AlignLeft, BarChart2, ChevronRight, BookOpen } from 'lucide-react';
...
// Helper to convert Sanity Portable Text blocks back into Markdown string
function parseBlocksToMarkdown(blocks: any[]): string {
...
export function EditPostForm({ post }: EditPostFormProps) {
  ...
  const initialMarkdown = parseBlocksToMarkdown(post.content);
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl md:text-2xl font-bold tracking-tight text-gray-900">Edit Blog Post</h1>
          <p className="text-gray-400 text-xs font-medium mt-0.5 truncate">Modify article parameters, body content, or status</p>
        </div>
            ...
            {activeTab === 'basic' && (
              ...
              >
                ...
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <input name="title" defaultValue={post.title} required className={inputCls} placeholder="E.g. Building VisionOS Interfaces with Clean React Code" />
                </div>

                {/* Slug */}
                <div className="space-y-1.5">
                  <label className={labelCls}>Slug (URL Slug) *</label>
                  <input name="slug" defaultValue={post.slug} required className={`${inputCls} font-mono`} placeholder="building-visionos-interfaces-clean-react" />
                </div>

                {/* Read Time */}
                <div className="space-y-1.5">
                  <label className={labelCls}>Read Time (Minutes)</label>
                  <input name="readTime" type="number" defaultValue={post.readTime} min={1} className={inputCls} />
                </div>

                {/* Tags */}
                <div className="space-y-1.5 md:col-span-2">
                  <label className={labelCls}>Tags (Comma separated)</label>
                  <input name="tags" defaultValue={post.tags?.join(', ') || ''} className={inputCls} placeholder="React, Next.js, UI/UX, Brutalism" />
                </div>
              </motion.div>
            )}
            ...
            {activeTab === 'content' && (
              ...
              >
                ...
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  <textarea name="excerpt" defaultValue={post.excerpt} required rows={2} maxLength={250}
                ...
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  <textarea name="content" defaultValue={initialMarkdown} required rows={10}
            ...
            {activeTab === 'publish' && (
              ...
              >
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50/80 rounded-2xl border border-gray-200/60">
                  <div>
                    <p className="text-sm font-bold text-gray-900">Publish Article</p>
                    <p className="text-xs text-gray-400 font-medium mt-0.5">Make this article publicly readable on your blog.</p>
                  </div>
                  <label className="flex items-center cursor-pointer">
                    <div className="relative">
                      <input type="checkbox" name="isPublic" className="sr-only peer" defaultChecked={post.isPublic} />
