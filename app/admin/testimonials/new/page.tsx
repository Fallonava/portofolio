...
export default function NewTestimonialPage() {
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      >
        <Link href="/admin/testimonials" className="p-2.5 bg-white/70 backdrop-blur-md rounded-xl border border-white/60 text-gray-500 hover:text-gray-900 hover:bg-white transition-all shadow-sm">
          <ArrowLeft size={18} />
        </Link>
        <div>
          <h1 className="text-xl md:text-2xl font-bold tracking-tight text-gray-900">Add Testimonial</h1>
          <p className="text-gray-400 text-xs font-medium mt-0.5">Showcase what clients say about your work</p>
        </div>
      </MotionDiv>
        ...
        <form onSubmit={handleSubmit} className="h-full flex flex-col p-6 md:p-8 gap-4">
          <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className={labelCls}>Designation / Company *</label>
                <input name="designation" required className={inputCls} placeholder="CEO at Apple" />
              </div>

              <div className="space-y-1.5 md:col-span-2">
                <label className={labelCls}>Order (Sorting)</label>
                <input name="order" type="number" defaultValue={0} className={`${inputCls} max-w-xs`} />
              </div>
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <textarea name="content" required rows={8}
                  ...
                  placeholder="Their review goes here..." />
              </div>
          ...
          <div className="shrink-0 pt-2 border-t border-gray-100">
            ...
            >
              {loading ? <><Loader2 size={18} className="animate-spin" />Saving...</> : <><Save size={18} />Add Testimonial</>}
            </button>
          </div>
        </form>
      </MotionDiv>
